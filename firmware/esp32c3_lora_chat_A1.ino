/* Nodo A1 - ESP32-C3 + RA-02 433MHz + WiFi Web UI */
#include <SPI.h>
#include <LoRa.h>
#include <WiFi.h>
#include <WebServer.h>
...
.header{padding:10px 12px;background:#111;border-bottom:1px solid #222;display:flex;align-items:center;gap:10px}
.title{font-size:18px;font-weight:bold;flex:1}
.status{color:#aaa;font-size:13px}
.settings-btn{width:32px;height:32px;border-radius:50%;background:#1a1a1a;border:1px solid #333;cursor:pointer;display:flex;align-items:center;justify-content:center}
...
.modal{display:none;position:fixed;inset:0;background:rgba(0,0,0,0.8);z-index:10;align-items:center;justify-content:center}
.modal.open{display:flex}
.modal-content{background:#111;border:1px solid #333;padding:16px;width:90%;max-width:400px}
.modal-header{display:flex;align-items:center;justify-content:space-between;margin-bottom:12px}
.modal-title{font-weight:bold}
.close-btn{background:none;border:none;color:#aaa;font-size:20px;cursor:pointer}
.section-modal{margin-bottom:12px}
.section-modal .section-title{margin-bottom:6px}
...
void setup() {
  Serial.begin(115200);
  delay(1500);
  Serial.println("Nodo A1 pronto (WiFi + LoRa)");
  setupLoRa();
  setupWiFi();
}
